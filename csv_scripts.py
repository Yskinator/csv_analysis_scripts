...
def match_commodities():
    rows = []
    with open('stock_with_segments.csv') as f:
        r = csv.DictReader(f)
...
def generate_preprocessed_stocks_csv():
    if os.path.isfile("preprocessed_stocks_with_brands.csv"):
        return
    descriptions = {}
    brands = {}
...
def count_brands():
    if os.path.isfile("brand_counts.csv"):
        return
    brands = {}
    with open("combined_stock_master_withbrands.csv", "r") as f:
        r = csv.DictReader(f)
...
def map_preprocessed_to_original():
    stocks = {}
    with open("combined_stock_master_withbrands.csv") as f1, open("stock_with_commodities.csv") as f2:
...
def add_commodities_to_stocks():
    ...
    count_brands()
    brands_to_segments.brands_to_segments()
