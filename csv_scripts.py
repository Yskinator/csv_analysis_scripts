...
def match_commodities(stock_with_segments, jlimit):
    rows = []
    brands = get_brands()
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = []
        for row in stock_with_segments:
            futures.append(executor.submit(match_commodities_for_row, row, brands, jlimit))
        for future in futures:
            updated_row = future.result()
            rows.append(updated_row)
    return rows

def match_commodities_for_row(row, brands = [], jlimit=0.3):
    d = row["Description"]
...
def add_commodities_to_stocks(stock_master):
    ...
    stock_with_commodities = match_commodities(stock_with_segments, 0.3) #number gives Jaccard index score threshold
